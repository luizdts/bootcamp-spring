...
public interface EmpregadoRepository extends JpaRepository<Empregado, Long> {
	...
	@Query(nativeQuery=true, value="SELECT empregados.cpf, empregados.enome, departamentos.dnome "
			...
			+ "WHERE empregados.cpf NOT IN (SELECT empregados.cpf FROM empregados INNER JOIN trabalha ON "
			+ "trabalha.cpf_emp = empregados.cpf) ORDER BY empregados.cpf")
	...
	@Query("SELECT new com.devsuperior.uri2990.dto.EmpregadoDeptDTO(obj.cpf, obj.enome, obj.departamento.dnome) "
			+ "FROM Empregado obj "
			+ "WHERE obj.cpf NOT IN (SELECT obj.cpf FROM Empregado obj INNER JOIN obj.projetosOndeTrabalha) ORDER BY obj.cpf")
